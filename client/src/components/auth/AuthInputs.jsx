...
export const InputGroup = ({ label, icon, type, value, onChange, placeholder, isMobileDark }) => {
    ...
    return (
        <div>
            <label htmlFor={id} className={`block text-xs font-bold uppercase tracking-wider mb-2 ml-1 ${isMobileDark ? 'text-slate-300 lg:text-slate-500' : 'text-slate-500'}`}>{label}</label>
...
const ReqItem = ({ met, text, isMobileDark }) => (
    <div className={`flex items-center gap-1.5 text-[10px] font-bold transition-colors duration-300 ${met ? "text-emerald-400 lg:text-emerald-600" : (isMobileDark ? "text-slate-500 lg:text-slate-300" : "text-slate-300")}`}>
        <div className={`size-3 rounded-full flex items-center justify-center border ${met ? "bg-emerald-500 border-emerald-500" : (isMobileDark ? "border-slate-600 lg:border-slate-200" : "border-slate-200")}`}>
