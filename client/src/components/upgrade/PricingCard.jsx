                    ...
                    <div className="relative p-8 md:p-12 lg:p-14 flex flex-col md:flex-row gap-12 lg:gap-20 items-center">
                        ...
                        <div className="flex-1 w-full text-center md:text-left relative z-10">
                            ...
                            <div className="flex items-end justify-center md:justify-start gap-3 mb-10 relative">
                                ...
                                <span className="text-7xl md:text-8xl font-black dark:from-white dark: dark: tracking-tighter text-slate-600 dark:text-slate-400">
                                    $29
                                </span>
                            ...
                            >
                                ...
                                <span className="relative size-full flex items-center justify-center gap-3 rounded-2xl bg-slate-900 dark:bg-slate-950 px-8 py-5 text-lg font-bold text-white backdrop-blur-3xl transition-all duration-300 group-hover/btn:bg-slate-800 dark:group-hover/btn:bg-slate-900">
                                    {loading ? (
                                        <Loader2 size={24} className="animate-spin text-indigo-400" />
                                    ) : (
                                        <>
                                            <span className="group-hover/btn:from-white group-hover/btn:to-white transition-all text-indigo-600 dark:text-indigo-400">
                        ...
                        <div className="flex-1 w-full">
                            ...
                            <ul className="space-y-5">
                                {features.map((feature, i) => (
                                    <m.li suppressHydrationWarning key={feature.id || feature.name || feature.title || crypto.randomUUID()}
                                        initial={{ opacity: 0, x: 20 }}
                                        animate={{ opacity: 1, x: 0 }}
                                        transition={{ delay: 0.3 + (i * 0.1) }}
                                        className="flex items-start gap-4 group/item"
                                    >
                                        <div className={`mt-0.5 flex-shrink-0 size-6 rounded-full flex items-center justify-center border ${feature.highlight ? 'bg-indigo-500 border-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400'}`}>
                            ...
                            <div className="mt-10 pt-8 border-t border-slate-200 dark:border-slate-800/50 flex items-center gap-4">
                                <div className="flex">
                                    {[1, 2, 3, 4].map((i, index) => (
                                        <div key={i} className={`size-8 rounded-full border-2 border-white dark:border-slate-900 bg-slate-200 dark:bg-slate-700 overflow-hidden ${index > 0 ? '-ml-3' : ''}`}>
                                            <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${i * 123}`} alt="user" className="size-full" />
                                        </div>
                                    ))}
                                </div>
