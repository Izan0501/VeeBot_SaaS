  ...
  return (
    <div className="lg:col-span-4 lg:sticky lg:top-28 h-fit z-10 space-y-6">
      ...
      <m.div variants={itemVars} className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none flex flex-col items-center text-center relative overflow-hidden">
        ...
        <div className={`relative size-24 rounded-full flex items-center justify-center text-4xl font-black mb-4 shadow-2xl ${isPremium ? 'bg-gradient-to-tr from-amber-300 to-orange-500 text-white ring-4 ring-white dark:ring-slate-700' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
          {formData.name ? formData.name.charAt(0).toUpperCase() : 'U'}
          {isPremium && <div className="absolute bottom-0 right-0 bg-white dark:bg-slate-900 p-1 rounded-full"><div className="bg-green-500 size-4 rounded-full border-2 border-white dark:border-slate-900"></div></div>}
        </div>
