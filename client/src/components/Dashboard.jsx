...
import {
    BarChart3, Trash2, Send, Loader2, Bot, User, UploadCloud, Filter, ChevronDown, Search, X
} from 'lucide-react';
...
const Dashboard = ({ isModalOpen, setIsModalOpen }) => {
    ...
    const mainScrollRef = useRef(null); // Ref para el contenedor principal

    // Ref para rastrear si el modal estaba abierto
    const prevModalOpen = useRef(isModalOpen);

    // --- EFECTOS ---

    // 1. SCROLL INICIAL ARRIBA (Esto soluciona que la página aparezca al medio)
    useEffect(() => {
    ...
    // 2. AUTO-SCROLL CHAT (Solo baja en el chat si hay mensajes nuevos)
    useEffect(() => {
    ...
    // 3. FETCH DE DATOS Y DETECCIÓN DE SUBIDA
    ...
    useEffect(() => {
        ...
        // Si el modal SE ACABA DE CERRAR -> Solo recargamos datos (sin scroll a la tabla)
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            ...
            <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 md:px-8 sticky top-0 z-20 shadow-sm transition-colors">
                ...
                <button
                    ...
                    className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all shadow-lg shadow-indigo-200 dark:shadow-none flex items-center gap-2 transform hover:-translate-y-0.5"
                >
                    <UploadCloud size={18} />
            ...
            {/* CONTENIDO SCROLLABLE (Ref Principal Aquí) */}
            <div
                ref={mainScrollRef}
                className="flex-1 overflow-auto p-4 md:p-8 space-y-8 custom-scrollbar"
            >

                {/* STATS */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                    <StatCard title="Total Candidatos" value={candidates.length} trend="Base Activa" icon={<User className="text-indigo-600 dark:text-indigo-400" size={20} />} />
                    <StatCard title="Top Talents" value={candidates.filter(c => c.score > 80).length} trend="Match > 80%" icon={<BarChart3 className="text-green-600 dark:text-green-400" size={20} />} />
                    <StatCard title="Motor IA" value="Llama 3.3" trend="Online" icon={<Bot className="text-purple-600 dark:text-purple-400" size={20} />} />
                </div>

                {/* --- CHAT IA --- */}
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-[600px] md:h-[800px] transition-all">

                    {/* Header Chat */}
                    <div className="bg-slate-50/80 dark:bg-slate-800/50 backdrop-blur border-b border-slate-100 dark:border-slate-800 p-5 flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="font-bold text-slate-800 dark:text-white text-base">VeeBot AI Recruiter</h3>
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Sistema RAG Activo</span>
                            </div>
                        </div>
                    </div>

                    {/* Área de Mensajes */}
                    <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 bg-gradient-to-b from-white to-slate-50 dark:from-slate-900 dark:to-slate-950">
                        {messages.map((msg) => (
                            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`flex gap-4 max-w-[100%] md:max-w-[92%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>

                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm mt-1 ${msg.role === 'user' ? 'bg-indigo-600' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700'}`}>
                                        {msg.role === 'user' ? <User size={18} className="text-white" /> : <Bot size={18} className="text-indigo-600 dark:text-indigo-400" />}
                                    </div>

                                    <div className={`p-5 rounded-2xl shadow-sm text-sm md:text-base leading-relaxed overflow-hidden ${msg.role === 'user'
                                        ...
                                        <ReactMarkdown
                                            components={{
                                                ...
                                                h3: ({ node, ...props }) => <h3 className="text-lg font-bold mt-4 mb-2 text-slate-900 dark:text-white" {...props} />
                                            }}
                                        >
                                            {msg.text}
                                        </ReactMarkdown>
                        ...
                        {isThinking && (
                            <div className="flex gap-4">
                                <div className="w-10 h-10 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center"><Loader2 size={18} className="text-indigo-600 animate-spin" /></div>
                                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-6 py-4 rounded-2xl rounded-tl-none shadow-sm flex gap-1.5 items-center">
                                    <span className="w-2 h-2 bg-slate-400 rounded-full animate-bounce"></span><span className="w-2 h-2 bg-slate-400 rounded-full animate-bounce delay-100"></span><span className="w-2 h-2 bg-slate-400 rounded-full animate-bounce delay-200"></span>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="p-5 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 transition-colors">
                        <form onSubmit={handleAskAI} className="relative flex items-center gap-3">
                            <input
                                ...
                                placeholder="Ej: ¿Quién sabe React y vive en CABA?"
                                className="flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 text-base rounded-xl py-4 px-5 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder-slate-400 dark:placeholder-slate-500"
                                disabled={isThinking}
                            />
                            <button type="submit" disabled={isThinking || !chatQuery.trim()} className="bg-indigo-600 hover:bg-indigo-700 text-white p-4 rounded-xl transition-all shadow-md hover:shadow-lg transform active:scale-95"><Send size={20} /></button>
                        </form>
                    </div>
                </div>
...
const StatCard = ({ title, value, trend, icon }) => (
    <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition-shadow">
        <div className="flex justify-between items-start mb-2"><div className="bg-slate-50 dark:bg-slate-800 p-2 rounded-lg">{icon}</div><span className="text-[10px] font-bold uppercase tracking-wider text-green-600 bg-green-50 dark:bg-green-900/20 dark:text-green-400 px-2 py-0.5 rounded-full border border-green-100 dark:border-green-900/30">{trend}</span></div>
