...
import {
    BarChart3, Trash2, Send, Loader2, Bot, User, UploadCloud, Filter, ChevronDown, Search, X, Lock, Crown, Sparkles
} from 'lucide-react';
...
const Dashboard = ({ isModalOpen, setIsModalOpen, userRole }) => {
    ...
    const executeDelete = async (id) => {
        ...
        } catch (error) { toast.error("Error al eliminar", { id: toastId }); }
    };

    const handleDelete = (id) => {
        toast((t) => (
            <div className="flex flex-col gap-3 min-w-[250px] dark:text-slate-200">
                <div className="font-medium">¿Borrar definitivamente?</div>
                <div className="flex gap-3 justify-end">
                    <button onClick={() => toast.dismiss(t.id)} className="text-sm text-slate-500 hover:text-slate-800 dark:hover:text-white">Cancelar</button>
    ...
    const handleAskAI = async (e) => {
        e.preventDefault();
        if (!isPremium) {
            toast.error("Desbloquea el plan Agency para chatear 🔒", { icon: '🔒', style: { borderRadius: '10px', background: '#1e293b', color: '#fff' } });
            return;
        }
    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            ...
            <div ref={mainScrollRef} className="flex-1 overflow-auto p-4 md:p-8 space-y-8 custom-scrollbar">
                ...
                {/* --- CHAT IA (CORREGIDO RESPONSIVE) --- */}
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-[600px] md:h-[800px] transition-all relative group">

                    {/* OVERLAY DE BLOQUEO (FIX: z-10 y anchos responsive) */}
                    {!isPremium && (
                        <div className="absolute inset-0 z-10 bg-slate-50/70 dark:bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 transition-all duration-500">
                            {/* Card de Bloqueo: max-w-sm para que no sea gigante, w-full para móvil */}
                            <div className="w-full max-w-xs md:max-w-sm bg-white dark:bg-slate-800 p-6 md:p-8 rounded-3xl shadow-2xl border border-indigo-100 dark:border-indigo-900/50 text-center transform scale-100 animate-in fade-in zoom-in duration-300">
                                ...
                                <button
                                    onClick={() => navigate('/settings')}
                                    className="w-full py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl font-bold text-sm md:text-base hover:shadow-lg hover:shadow-indigo-500/30 transition-all active:scale-95 flex justify-center items-center gap-2"
                                >
                                    ...
                                    <div className={`p-4 md:p-5 rounded-2xl shadow-sm text-sm md:text-base leading-relaxed overflow-hidden ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-tl-none'}`}>
                                        <ReactMarkdown
                                            components={{
                                                ...
                                                h3: ({ node, ...props }) => <h3 className="text-base md:text-lg font-bold mt-2 mb-1" {...props} />
                                            }}
                                        >
                                            {msg.text}
                                        </ReactMarkdown>
                    ...
                    <div className="p-4 md:p-5 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 transition-colors">
                        <form onSubmit={handleAskAI} className="relative flex items-center gap-3">
                            <input
                                ...
                                placeholder={isPremium ? "Ej: ¿Quién sabe React y vive en CABA?" : "🔒 Desbloquea para chatear con la IA"}
                                className={`flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 text-sm md:text-base rounded-xl py-3 md:py-4 px-5 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder-slate-400 dark:placeholder-slate-500 ${!isPremium ? 'opacity-50 cursor-not-allowed' : ''}`}
                                disabled={isThinking || !isPremium}
                            />
                            <button
                                type="submit"
                                disabled={isThinking || !chatQuery.trim() || !isPremium}
                                className={`bg-indigo-600 text-white p-3 md:p-4 rounded-xl transition-all shadow-md transform ${!isPremium ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-700 hover:shadow-lg active:scale-95'}`}
                            >
