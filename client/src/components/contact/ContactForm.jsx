...
const ContactForm = ({ formData, handleFieldChange, submitContactForm, loading, isPublic, wrapperClasses }) => (
    <m.div
    ...
    >
        ...
        <div className="w-full max-w-lg space-y-6 md:space-y-8 relative z-10">
            ...
            <form onSubmit={submitContactForm} className="space-y-4 md:space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <FormInput label="Nombre" name="firstName" placeholder="Tu nombre" value={formData.firstName} onChange={handleFieldChange} delay={0.1} />
                    <FormInput label="Apellido" name="lastName" placeholder="Tu apellido" value={formData.lastName} onChange={handleFieldChange} delay={0.2} />
                </div>
                <FormInput label="Email Corporativo" name="email" type="email" placeholder="nombre@empresa.com" value={formData.email} onChange={handleFieldChange} delay={0.3} />

                <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                    <label htmlFor="message" className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 ml-1 block">Mensaje</label><textarea id="message"
                        required
                        name="message"
