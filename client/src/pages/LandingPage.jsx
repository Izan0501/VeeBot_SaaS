  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 overflow-x-hidden selection:bg-indigo-100 selection:text-indigo-900">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-1">
            {['Características', 'Planes'].map((item) => (
              <a
                key={item}
                href={item === 'Características' ? '#features' : '#pricing'}
                onClick={(e) => handleScroll(e, item === 'Características' ? 'features' : 'pricing')}
                className="relative px-5 py-2 text-sm font-medium text-slate-600 hover:text-indigo-600 transition-colors group overflow-hidden rounded-full"
              >
                <span className="relative z-10">{item}</span>
                {/* Fondo sutil al hover */}
                <span className="absolute inset-0 bg-indigo-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-full"></span>
              </a>
            ))}
            ...
            <button
              ...
              className="text-sm font-bold text-slate-700 hover:text-indigo-600 px-5 py-2 transition-colors"
            >
              Login
            </button>
            ...
            <button
              ...
              className="relative group bg-slate-900 text-white px-6 py-2.5 rounded-full text-sm font-bold overflow-hidden shadow-lg shadow-slate-900/20 hover:shadow-indigo-500/30 transition-all hover:-translate-y-0.5 active:scale-95"
            >
              <span className="relative z-10 flex items-center gap-2">
                Empezar Gratis <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </span>
        ...
        {/* MENU MÓVIL (ANIMADO) */}
        <div
          className={`md:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-xl border-b border-slate-200 shadow-xl overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] origin-top ${isMobileMenuOpen ? 'max-h-[400px] opacity-100' : 'max-h-0 opacity-0'
            }`}
        >
          <div className="flex flex-col p-6 space-y-2">
            <a href="#features" onClick={(e) => handleScroll(e, 'features')} className="text-lg font-medium text-slate-700 p-4 hover:bg-slate-50 rounded-2xl transition-colors flex justify-between items-center group">
              Características <ArrowRight size={16} className="text-indigo-400 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all" />
            </a>
            <a href="#pricing" onClick={(e) => handleScroll(e, 'pricing')} className="text-lg font-medium text-slate-700 p-4 hover:bg-slate-50 rounded-2xl transition-colors flex justify-between items-center group">
              Planes <ArrowRight size={16} className="text-indigo-400 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all" />
            </a>
            <Link to="/terms" className="text-lg font-medium text-slate-700 p-4 hover:bg-slate-50 rounded-2xl transition-colors flex justify-between items-center group">
              Terminos <ArrowRight size={16} className="text-indigo-400 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all" />
            </Link>
            <div className="h-px bg-slate-100 my-2"></div>
            <button onClick={() => handleNavigation('/login')} className="w-full py-3.5 text-center font-bold text-slate-700 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
              Iniciar Sesión
            </button>
            <button onClick={() => handleNavigation('/register')} className="w-full py-3.5 text-center font-bold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-colors active:scale-95">
              Empezar Gratis
            </button>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            <div className="bg-slate-100 p-1.5 rounded-lg">
              <BrainCircuit className="text-slate-900" size={20} />
            </div>
          ...
          <p className="text-sm">© 2026 VeeBot Inc.</p>
        </div>
      </footer>
    </div>
  );
