...
const Contact = ({ isPublic = true }) => {
    ...
    const wrapperClasses = isPublic
        ? "flex flex-col lg:flex-row w-full min-h-screen pt-20 bg-slate-50 dark:bg-slate-950"
        : "flex flex-col xl:flex-row w-full h-full min-h-[calc(100vh-100px)] bg-white dark:bg-slate-900 rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-sm";
    ...
    if (isPublic) {
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white transition-colors duration-300 selection:bg-pink-500 selection:text-white overflow-x-hidden flex flex-col">
                <motion.button
                    initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}
                    onClick={handleBack}
                    className="fixed top-24 left-4 lg:left-8 z-40 p-3 rounded-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-slate-200 dark:border-slate-800 shadow-lg hover:scale-105 transition-all group"
                >
